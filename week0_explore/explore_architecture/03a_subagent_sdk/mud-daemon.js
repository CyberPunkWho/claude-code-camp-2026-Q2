...
async function connect() {
  return new Promise((resolve) => {
    try {
      log('Starting connection to ' + CONFIG.host + ':' + CONFIG.port + ' via WSL netcat');

      // Use netcat via WSL with longer timeout
      ncProcess = spawn('wsl', ['-d', 'Ubuntu-24.04', 'nc', '-w', '60', CONFIG.host, CONFIG.port.toString()]);
      ...
      const loginTimeout = setTimeout(() => {
        ...
        if (!loginComplete) {
          loginComplete = true;
          try { ncProcess.kill(); } catch (e) {}
          resolve({ success: false, error: 'Login timeout' });
        }
      }, 45000);

      // Collect data from MUD
      ncProcess.stdout.on('data', (buffer) => {
        receivedData = Buffer.concat([receivedData, buffer]);
        const text = buffer.toString('binary');
        if (text && text.trim()) {
          log('MUD: ' + text.substring(0, 80).replace(/\n/g, ' '));
        }
      });

      ncProcess.on('error', (err) => {
        log(`NC error: ${err.message}`);
        isConnected = false;
      ...
      ncProcess.on('close', (code) => {
        log(`NC process closed with code ${code}`);
        if (!loginComplete && code !== 0) {
          loginComplete = true;
          resolve({ success: false, error: `NC closed unexpectedly (code ${code})` });
        }
      });
      ...
      const loginSequence = async () => {
        try {
          // Give netcat time to connect and MUD time to send welcome message
          await new Promise(r => setTimeout(r, 2500));

          log('Sending name: ' + CONFIG.character);
          ncProcess.stdin.write(CONFIG.character + '\r\n');

          // Wait for password prompt (MUD is slow with protocol detection)
          await new Promise(r => setTimeout(r, 2500));

          log('Sending password: ' + CONFIG.password);
          ncProcess.stdin.write(CONFIG.password + '\r\n');

          // Wait for login to settle and MUD to finish initialization
          ...
          log('Login complete via netcat');
          parseOutput(receivedData);
          saveState();
          savePlayerMarkdown();
          saveWorldMarkdown();
          resolve({ success: true, message: 'Connected and logged in via netcat' });
...
async function sendCommand(command) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      ncProcess.stdout.on('data', dataHandler);

      let resolved = false;
      const timeout = setTimeout(() => {
        if (!resolved) {
          resolved = true;
          ncProcess.stdout.removeListener('data', dataHandler);
