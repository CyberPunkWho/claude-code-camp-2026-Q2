...
The skill maintains persistent state in two places:

### Real-time State (`.mud-state/game-state.json`)
- **Character**: Name, level, XP, HP/mana/moves
- **Location**: Current room description and exits
- **Inventory & Equipment**: What you're carrying and wearing
...
The skill auto-exports your progress to human-readable markdown files:

**`data/player.md`** - Your character progress:
- Current stats (level, XP, HP, mana, moves)
- Current location
- Inventory and equipment
- Active goals and sub-goals
- Location history (last 10 rooms)

**`data/world.md`** - World knowledge base:
- Rooms you've discovered with descriptions and exits
- NPCs and their locations
- Monsters and their stats
- Items found and their locations

Access current state anytime with `/mud status`. Files update automatically after each command.

## Goal Tracking

Set and track longer-term objectives. Goals persist across sessions and drive your progression.

### Goal Commands

**Set a goal:**
```
/mud goal set "Reach level 7"
/mud goal set "Defeat the Ogre Mage"
/mud goal set "Collect all elemental stones"
```
...
- **Persistence**: Goals are saved to `data/player.md` and survive disconnects
- **Sub-goals**: Complex goals can be broken into steps (use `/mud goal set` multiple times)
- **Status tracking**: Each goal shows completion status (✓ or ○)
- **Guidance**: While playing, the skill can suggest actions aligned with your active goals

### Example: Multi-step Goal

```
1. /mud goal set "Reach level 5"          # Sub-goal 1
2. /mud goal set "Find the Sword of Dawn" # Sub-goal 2
3. /mud goal set "Defeat the Dragon"      # Main goal

# Now explore and fight to complete these in order
```

The skill uses these goals to suggest relevant commands and alert you when you make progress toward them.
...
### Memory & Goals
- **Player memory**: Character progress synced to `data/player.md` after each command
- **World memory**: Discovered locations and creatures synced to `data/world.md`
- **Goal tracking**: Goals persist in both JSON and markdown formats
- **Manual edits**: You can directly edit `data/player.md` and `data/world.md` to fix or add information
