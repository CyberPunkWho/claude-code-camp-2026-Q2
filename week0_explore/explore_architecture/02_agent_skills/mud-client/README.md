...
✅ **Goal Tracking** - Set and track long-term objectives like "Reach level 7"  
✅ **Persistent Memory** - Character progress and world knowledge saved to `data/player.md` and `data/world.md`  
...
- **references/** - Setup & advanced docs
- **evals/** - Test cases
- **data/** - Persistent memory files
  - **data/player.md** - Character progress and goals
  - **data/world.md** - World knowledge base
- **.mud-state/** - Runtime state and logs
...
The daemon maintains and saves state in two formats:

**Real-time State** (`.mud-state/game-state.json`):
- Character stats (HP, mana, moves, level, XP)
- Current location and room description
- Inventory and equipped items
- Combat status and active targets
- Map of explored areas

**Persistent Memory** (`data/` directory):
- **data/player.md** - Human-readable character progress, inventory, goals, and location history
- **data/world.md** - World knowledge: discovered rooms, NPCs, monsters, and items

The markdown files update automatically after each command and let you:
- Track your progress across sessions
- Read and edit your goals manually
- Review what you've learned about the world
- Use as reference material while playing
...
# Goals & Progress
/mud goal set "Reach level 7"        # Create a goal
/mud goal set "Defeat the Ogre Mage" # Add another goal
/mud goal list                       # Show all goals
/mud goal complete 0                 # Mark first goal as complete

# Status
/mud score                  # Character stats
/mud equipment              # Show equipped items
/mud experience             # Check XP progress
/mud status                 # Full game state
```
