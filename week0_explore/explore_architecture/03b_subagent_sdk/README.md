...
03_subagent_sdk/
├── .claude/
│   ├── agents.js                   ← Agent definitions (programmatic SDK)
...
├── run_agent.py                    ← Python agent runner (interactive CLI)
...
Use the agent directly in Claude Code:
```
...
### Option 2: Python Interactive Runner
Run agents programmatically with an interactive CLI:
...
Or type an agent name directly: mud-client-agent
```

The Python runner provides:
- Interactive stdin loop for multi-turn conversations
- Persistent conversation history per agent
- Extended thinking with configurable reasoning budgets
- Direct Claude API integration using AgentDefinition
